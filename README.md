...
5. 「履歴」画面の「LINE用テキストを生成」から、90秒超または例外メモのある精算済み記録だけの報告文を作成し、「コピー」でクリップボードへコピーできます。90秒以内の正常記録、発行済み・精算待ち、駐車中の記録は含まれません。
...
記録中・発行済み・履歴の各画面にある「編集」「削除」は同じ記録データへ反映されます。記録中・発行済みの番号を変更すると、表示・精算待ち一覧・履歴・LINE用テキストにも反映されます。LINE用テキストは履歴画面の精算済み記録から、90秒超または例外メモのあるものだけを対象にします。記録を削除すると表示中の生成済みLINE文面もクリアされ、残った記録で再生成できます。削除は確認ダイアログ付きです。
